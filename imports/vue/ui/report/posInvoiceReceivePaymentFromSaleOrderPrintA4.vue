<!--suppress ALL -->
<template>
    <div class="posInvoiceReceivePaymentFromSaleOrderPrintA4-report">
        <a4>
            <span slot="content" style="margin: 0px !important;">
                ...
                <thead style="margin-top: 5px">
                    <tr>
                        <td>{{langConfig['no']}}</td>
                        <td>{{langConfig['orderDate']}}</td>
                        <td>{{langConfig['orderNo']}}</td>
...
<script>
    ...
    export default {
        ...
        methods: {
            handleRun(invoiceId) {
                this.loading = true;
                Meteor.call('posInvoiceReceivePaymentFromSaleOrderPrintA4Report', invoiceId, this.langConfig, (err, result) => {
                    if (result) {
