<!--suppress ALL -->
<template>
    <div class="posSaleReceivePaymentByCustomerDetail-report">
        <a4>
                ...
                <el-row type="flex" class="row-bg" justify="center">
                    <el-col :span="24">
                        <el-card class="box-card">
                            ...
                            <el-form :model="params" :label-position="labelPosition" :rules="rules" :ref="refForm">
                                <el-row type="flex" class="row-bg" justify="center">
                                    <el-col>
...
<div class="row"
  ...
  <p class="row"
     style="text-align: center !important;font-family:'Khmer OS Battambang';font-size: 11 !important;">
                ...
                <tbody style="margin-bottom: 5px;" v-html="saleReceivePaymentHtml">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            handleRun() {
                this.$refs["saleReceive"].validate((valid) => {
                    if (valid) {
                        this.loading = true;
                        ...
                        Meteor.call('posSaleReceivePaymentByCustomerDetailReport', this.params, this.langConfig, (err, result) => {
                            if (result) {
                                this.saleReceivePaymentHtml = result.saleReceivePaymentHTML;
        ...
        computed: {
            dataExist() {
                // return this.posSaleReceivePaymentData.length > 0;
            },
            langConfig() {
                let data = compoLangReport.filter(config => config.lang === this.langSessionReport)[0]['saleReceivePaymentByCustomerDetail'];
                return data;
            }
        },
        components: {
