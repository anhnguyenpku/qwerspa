<template>
    <div class="meterReadingJournalThreeMonth-report">
        <div class="no-print">
            <div slot="header" class="no-print">
                <el-row type="flex" class="row-bg" justify="center">
                    <el-col :span="24">
                        ...
                        <span slot="title">
                            Meter Reading Journal​​ Three Month Filter <i class="header-icon el-icon-information"></i>
                        </span>
                ...
                <div v-for="doc in reportData">
                    <!--<div class="row">
                        ...
                        <div class="pull-right">
                            <h5>{{userNamePrint}}</h5>
                            <h5>{{datePrint}}</h5>
                            &lt;!&ndash;<span id="page-number"></span>&ndash;&gt;
                        </div>
                    </div>-->
                    <table style="border-bottom: 2px solid #000;">
                        ...
                        <tr class="b-top-bottom" style="border-top: 2px solid #000">
                            ...
                            <th style=" width: 3cm !important">{{val1 | switchMonth}}</th>
                            <th style=" width: 3cm !important">{{val2 | switchMonth}}</th>
                            <th style=" width: 3cm !important">{{val3 | switchMonth}}</th>
                        ...
                        <tr v-for="journalDetail in doc.meterReadingJournalDetail"
                            style="border-bottom: 1px solid #48576A;font-size: 10px;">

                            <td>{{isOldBlockExist(journalDetail)}}</td>
                            <td style="text-align: left !important;font-family: 'Khmer OS Battambang';font-size: 12px;" >{{journalDetail.customerDoc.khName}}</td>
                            ...
                            <td style="text-align: left !important;">{{journalDetail.meterDoc.diameter}}&nbsp;&nbsp;{{journalDetail.customerDoc.contract.meterSerialNo}}</td>
                            ...
                            <td style="border-left: 0px !important;font-size: 11px;">
                    ...
                    shortcuts: [
                        {
                            text: 'Quarter 1',
                            onClick(picker) {
                                const end = moment().startOf("year").add(3,"months").add(-1,"days").toDate();
                            ...
                            text: 'Quarter 2',
                            onClick(picker) {
                                const end = moment().startOf("year").add(6,"months").add(-1,"days").toDate();
                                const start = moment().startOf("year").add(3,"months").toDate();
                                picker.$emit('pick', [start, end]);
                            }
                        }, {
                            text: 'Quarter 3',
                            onClick(picker) {
                                const end = moment().startOf("year").add(9,"months").add(-1,"days").toDate();
                                const start = moment().startOf("year").add(6,"months").toDate();
                                picker.$emit('pick', [start, end]);
                            }
                        }, {
                            text: 'Quarter 4',
                            onClick(picker) {
                                ...
                                const start = moment().startOf("year").add(9,"months").toDate();
                                picker.$emit('pick', [start, end]);
                            }
                        },

                        {
                            text: 'Q1 Last Year',
                            onClick(picker) {
                                const end = moment().add(-1,"year").startOf("year").add(3,"months").add(-1,"days").toDate();
                            ...
                            text: 'Q2 Last Year',
                            onClick(picker) {
                                const end = moment().add(-1,"year").startOf("year").add(6,"months").add(-1,"days").toDate();
                                const start = moment().add(-1,"year").startOf("year").add(3,"months").toDate();
                                picker.$emit('pick', [start, end]);
                            }
                        }, {
                            text: 'Q3 Last Year',
                            onClick(picker) {
                                const end = moment().add(-1,"year").startOf("year").add(9,"months").add(-1,"days").toDate();
                                const start = moment().add(-1,"year").startOf("year").add(6,"months").toDate();
                                picker.$emit('pick', [start, end]);
                            }
                        }, {
                            text: 'Q4 Last Year',
                            onClick(picker) {
                                ...
                                const start = moment().add(-1,"year").startOf("year").add(9,"months").toDate();
                                picker.$emit('pick', [start, end]);
                            }
                        }]
                },
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        if (this.journalReading.meterQuery.length > 0) {
                            ...
                            this.val1= moment(this.dateRange[0]).format("MM");
                            this.val2= parseInt(moment(this.dateRange[0]).format("MM"))+1+"";
                            this.val3= parseInt(moment(this.dateRange[0]).format("MM"))+2+"";
