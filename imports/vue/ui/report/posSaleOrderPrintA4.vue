<!--suppress ALL -->
<template>
    <div class="posSaleOrderPrintA4-report">
        <a4>
            <span slot="content" style="margin: 0px !important;">
                <div class="printSaleOrder">
                      ...
                      <caption style="padding-top: 0px !important;">
                          <div class="row">
                              ...
                              <span style="text-align: center;font-size: 15px; border: 0px !important; float: right">
                                  ...
                                  SaleOrder<p class="saleOrderNo">{{saleOrderDoc && saleOrderDoc.saleOrderNo || ""}}</p>
                              </span>
                              </div>
                          </div>
                          <br>
                          <div style="width: 100%; float: right">
                              <div style="width: 70% !important; float: left">
                                  {{langConfig['customer']}} : {{saleOrderDoc && saleOrderDoc.customerDoc && saleOrderDoc.customerDoc.name || ""}}
                              </div>
                              <div style="width: 30% !important;float: right">
                                  {{langConfig['date']}} : {{saleOrderDoc && saleOrderDoc.saleOrderDateName || ""}}
                              </div>
                          </div>
                          <div style="width: 100%">
                              <div style="width: 70% !important;float: left">
                                  {{langConfig['address']}} : {{saleOrderDoc && saleOrderDoc.customerDoc && saleOrderDoc.customerDoc.address || ""}}
                              </div>
                                <div style="width: 30% !important;float: right">
                                  {{langConfig['phoneNumber']}} : {{saleOrderDoc && saleOrderDoc.customerDoc && saleOrderDoc.customerDoc.phoneNumber || ""}}
                              </div>
                          </div>
                      </caption>
                ...
                <tbody style="margin-bottom: 5px;" v-html="printSaleOrderA4Html">
                </tbody>
...
<script>
    ...
    export default {
        ...
        methods: {
            handleRun(saleOrderId) {
                this.loading = true;
                Meteor.call('posSaleOrderPrintA4Report', saleOrderId, this.langConfig, (err, result) => {
                    if (result) {
                        this.printSaleOrderA4Html = result.printSaleOrderA4Html;
                        this.saleOrderDoc = result.saleOrderDoc;
                        this.loading = false;
                    } else {
                        this.loading = false;
                    }
        ...
        updated: function () {
            ...
            vm.$nextTick(function () {
                ...
                if (vm.printSaleOrderA4Html != "" && vm.onLoad == true) {
                    setTimeout(function () {
                        window.print();
                        FlowRouter.go('/pos-sale/posSaleOrder');
                        vm.printSaleOrderA4Html = "";
                        vm.$message({
                            duration: 1000,
        ...
        computed: {
            langConfig() {
                let data = compoLangReport.filter(config => config.lang === this.langSessionReport)[0]['saleOrderA4Print'];
                return data;
            }
        },
        components: {
