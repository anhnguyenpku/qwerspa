<!--suppress ALL -->
<template>
    <div class="unpaid-customer-block-expire-report">
        <a4>
            <div slot="header">
                <el-row type="flex" class="row-bg no-print" justify="center">
                    <el-col :span="24">
                        ...
                        <span slot="title">
                            Unpaid Customer Block Expire Report Filter <i class="header-icon el-icon-information"></i>
                        </span>
                                <el-form :inline="true">
                                    <el-row type="flex" class="row-bg" justify="center">
                                        ...
                                        <el-form-item label="Divide">
                                            <el-select filterable v-model="params.cutStatus" placeholder="All">
                                                <el-option
                                                        v-for="item in cutStatusOptions"
                                                        :label="item.label"
                                                        :value="item.value" :key="item._id">
                                                </el-option>
                                            </el-select>
            ...
            <span slot="content">
                ...
                <div class="row">
                        <div class="col-md-12"
                            ...
                            របាយការណ៍ អតិថិជនជំពាក់តាមប្លុក ផុតកំណត់
                        </div>
                    </div>
                <div class="row">
                    ...
                    <table class="table-custom" style="font-size: 11px;">
                        ...
                        <tbody>
                         <slot v-for="block in unpaidCustomers.data">
                             <tr style="font-family: 'Khmer OS Muol';text-decoration: underline">
                                        <td colspan="6">វិក្កយបត្រ​ក្នុងតំបន់ៈ {{block.districtCode}}{{block.quartierDoc.code}}{{block.blockDoc.code}} {{block.quartierDoc.name}}</td>
                                 <td class="text-center">{{block.totalWaterConsumption | numFormatRiel}}</td>
                        ...
                        <slot v-for="(detail,index) in block.data">
                                     ...
                                     <tr>
                                         <th></th>
                                         <th>{{index + 1}}</th>
                                        <th style="font-family: 'Khmer OS Battambang';" colspan="4">{{detail.customerDoc.khName}}</th>
                                         <!--<td>{{detail.streetNo}}</td>
                                         <td>{{detail.dpc}}</td>-->
                                         <!--<td>{{detail.customerDoc.streetNo}}</td>-->
                                         <!--<td class="text-center">{{detail.newReadingDate | momentFormat}}</td>-->
                                        <th class="text-center">{{detail.totalWaterConsumption | numFormatRiel}}</th>
                                        <th class="text-right">{{detail.totalMaintenanceFee | numFormatRiel}}</th>
                                        <th class="text-right">{{detail.totalContributionFee | numFormatRiel}}</th>
                                        <th class="text-right">{{detail.totalWaterUsage | numFormatRiel}}</th>
                                        <th class="text-right">{{detail.totalBalance | numFormatRiel}}</th>
                                        <th class="text-center" style="font-family:'Khmer OS Battambang';">
                                        </th>
                                         <th class="text-center" style="font-family:'Khmer OS Battambang';">
                                        </th>
                                         <!--  <td>{{detail.balance}}</td>-->
                                    </tr>
                                    <slot v-for="(subDetail,subIndex) in detail.data">

                                    <tr>
                                        <td></td>
                                        <td></td>
                                        <td> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{{subDetail.newReadingDate | switchMonthYearToKh}}</td>
                                        <td>{{subDetail.streetNo}}</td>
                                        <td>{{subDetail.dpc}}</td>
                                        <td>{{subDetail.barcode}}</td>
                                        <!--<td>{{detail.customerDoc.streetNo}}</td>-->
                                        <!--<td class="text-center">{{detail.newReadingDate | momentFormat}}</td>-->
                                        <td class="text-center">{{subDetail.waterConsumption | numFormatRiel}}</td>
                                        <td class="text-right">{{subDetail.maintenanceFee | numFormatRiel}}</td>
                                        <td class="text-right">{{subDetail.contributionFee | numFormatRiel}}</td>
                                        <td class="text-right">{{subDetail.total | numFormatRiel}}</td>
                                        <td class="text-right">{{subDetail.balance | numFormatRiel}}</td>
                                        <td class="text-center" style="font-family:'Khmer OS Battambang';">
                                            <span v-if="getInvoiceType(subDetail.newReadingDate)">ផុតកំណត់</span>
                                            <span v-else>ធម្មតា</span> ({{(subDetail.dayExpire/ 86400000) -waterBillingSetup.invoiceExpiredAfter | numFormatRiel}} ថ្ងៃ)
                                        </td>
                                        <td style="font-family:'Khmer OS Battambang';">{{subDetail.closedAt | momentFormat}} {{subDetail.desc}}</td>
                                    </tr>
                                    </slot>
                        ...
                        <tr style="background: #f5f5f5;font-weight: bold">
                            <td colspan="6" style="font-weight: normal;font-family: 'Khmer OS Muol'">សរុប:</td>
                    ...
                    <table width="100%">
                        <tr style="font-weight: bold;font-size:11px; line-height: 15px;font-family: Khmer os Battambang;">
                            ...
                            <td colspan="3" style="border: none !important;line-height:16px;">
    ...
    export default {
        ...
        methods: {
            ...
            getInvoiceType(newReadingDate) {
                let add25days = moment(newReadingDate).startOf('days').add(25, 'days').format('YYYY-MM-DD');
                let date = moment(this.params.date).startOf('days');
                let expired = moment(date).isAfter(add25days);
                return expired;
            },
            exportToExcel() {
            ...
            handleRun() {
                ...
                //----------------------------//
                let selector2 = {};
                if (this.params.cutStatus != "") {
                    selector2["requestCuttingWaterDoc.closedAt"] = this.params.cutStatus == "cut" ? {$exists: true} : {$exists: false};
                }
                //---------------------------//

                Meteor.call('unpaidCustomerBlockExpireReport', this.params, selector2, (err, result) => {
                    if (result) {
