<!--suppress ALL -->
<template>
    <div class="unpaid-customer-expire-report">
        <a4>
            <div slot="header">
                <el-row type="flex" class="row-bg no-print" justify="center">
                    <el-col :span="24">
                        ...
                        <span slot="title">
                            Unpaid Customer Expire Report Filter <i class="header-icon el-icon-information"></i>
                        </span>
                                <el-form :inline="true">
            ...
            <span slot="content">
                ...
                <div class="row">
                        <div class="col-md-12"
                            ...
                            របាយការណ៍ អតិថិជនជំពាក់ ផុតកំណត់
                        </div>
                    </div>
                <div class="row">
                        ...
                        <tbody>
                         <slot v-for="date in unpaidCustomers.data">
                             <tr style="font-family: 'Khmer OS Muol';text-decoration: underline">
                                        <td colspan="12">វិក្កយបត្រ​ក្នុងខែៈ {{date.month}}/{{date.year}}</td></tr>
                        <slot v-for="(detail,index) in date.data">
                                     ...
                                     <tr>
                                        ...
                                        <td class="text-center" style="font-family:'Khmer OS Battambang';">
                                            ...
                                            <span v-else>ធម្មតា</span>({{(detail.dayExpire/ 86400000)-waterBillingSetup.invoiceExpiredAfter | numFormatRiel}} ថ្ងៃ)
                                        </td>
    ...
    export default {
        ...
        methods: {
            ...
            getInvoiceType(newReadingDate) {
                let add24days = moment(newReadingDate).startOf('days').add(25, 'days').format('YYYY-MM-DD');
            ...
            handleRun() {
                ...
                if (this.params.divideUpDown != "") {
                    selector2.street = this.params.divideUpDown == "up" ? {$gt: "480"} : {$lte: "480"};
                }
                //---------------------------//

                Meteor.call('unpaidCustomerExpireReport', this.params, selector2, (err, result) => {
                    if (result) {
